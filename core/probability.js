...
function fillArrayOfItems(pItem, pArrayToFill) {
	if (pItem.getOccurences > 0) {
		for (let index = 0; index < pItem.getOccurences; index++) {
...
function testRun(n, pArrayStartItems, pArrayFilled, shouldMixArrayEverytime = false) {
	...
	pArrayStartItems.forEach((e)=>{
		total += e.getOccurences/pArrayFilled.length; 
		console.log(e.getName + " : " + e.getOccurences/pArrayFilled.length + " (" + e.getOccurences + " / " + pArrayFilled.length + ")");
	});
